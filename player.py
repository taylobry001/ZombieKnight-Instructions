...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, platform_group, portal_group, bullet_group):
        ...
        # Animation frames
        self.move_right_sprites = load_frames(
            "images/player/run",
            RUN_FRAMES,
            (64, 64)
        )

        self.move_left_sprites = flip_frames(
            self.move_right_sprites
        )

        self.idle_right_sprites = load_frames(
            "images/player/idle",
            IDLE_FRAMES,
            (64, 64)
        )

        self.idle_left_sprites = flip_frames(
            self.idle_right_sprites
        )

        self.jump_right_sprites = load_frames(
            "images/player/jump",
            JUMP_FRAMES,
            (64, 64)
        )

        self.jump_left_sprites = flip_frames(
            self.jump_right_sprites
        )

        self.attack_right_sprites = load_frames(
            "images/player/attack",
            ATTACK_FRAMES,
            (64, 64)
        )

        self.attack_left_sprites = flip_frames(
        ...
        if self.velocity.y != 0:
            ...
            if collided_platforms:
                ...
                if self.velocity.y > 0:
                    # Landing on platform
                    self.position.y = (
                        collided_platforms[0].rect.top + 5
                    )
    ...
    def animate(self, sprite_list, speed):
        ...
        self.current_sprite, wrapped = advance_frame(
            self.current_sprite,
            sprite_list,
            speed
        )
